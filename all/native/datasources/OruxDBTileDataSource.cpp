...
#include "OruxDBTileDataSource.h"
...
namespace carto {
    ...
    OruxDBTileDataSource::OruxDBTileDataSource(int minZoom, int maxZoom, const std::string& path, MBTilesScheme::MBTilesScheme scheme) :
        TileDataSource(minZoom, maxZoom),
        _scheme(scheme),
        _database(OpenDatabase(path)),
        _cachedMinZoom(minZoom),
    ...
    OruxDBTileDataSource::~OruxDBTileDataSource() {
    }
    
    std::map<std::string, std::string> OruxDBTileDataSource::getMetaData() const {
        std::lock_guard<std::recursive_mutex> lock(_mutex);
        if (!_database) {
            Log::Error("OruxDBTileDataSource::getMetaData: Not connected to the database");
        ...
        try {
            ...
            Log::Errorf("OruxDBTileDataSource::getMetaData: Failed to query metadata from the database: %s", ex.what());
            return std::map<std::string, std::string>();
        }
    }

    int OruxDBTileDataSource::getMinZoom() const {
        std::lock_guard<std::recursive_mutex> lock(_mutex);

        // If value is not cached, load from database
        if (!_cachedMinZoom) {
            if (!_database) {
                Log::Error("OruxDBTileDataSource::getMinZoom: Not connected to the database");
                return 0;
            }
    ...
    int OruxDBTileDataSource::getMaxZoom() const {
        std::lock_guard<std::recursive_mutex> lock(_mutex);

        // If value is not cached, load from database
        if (!_cachedMaxZoom) {
            if (!_database) {
                Log::Error("OruxDBTileDataSource::getMaxZoom: Not connected to the database");
                return -1;
            }
    ...
    MapBounds OruxDBTileDataSource::getDataExtent() const {
        std::lock_guard<std::recursive_mutex> lock(_mutex);

        // If value is not cached, load from database
        if (!_cachedDataExtent) {
            if (!_database) {
                Log::Error("OruxDBTileDataSource::getDataExtent: Not connected to the database");
                return MapBounds();
            }
    ...
    std::shared_ptr<TileData> OruxDBTileDataSource::loadTile(const MapTile& mapTile) {
        std::lock_guard<std::recursive_mutex> lock(_mutex);
        Log::Infof("OruxDBTileDataSource::loadTile: Loading %s", mapTile.toString().c_str());
        if (!_database) {
            Log::Errorf("OruxDBTileDataSource::loadTile: Failed to load %s: Couldn't connect to the database", mapTile.toString().c_str());
        ...
        try {
            ...
            if (it == query.end()) {
                ...
                if (mapTile.getZoom() > getMinZoom()) {
                    Log::Infof("OruxDBTileDataSource::loadTile: Tile data doesn't exist in the database, redirecting to parent");
                    tileData->setReplaceWithParent(true);
                } else {
                    Log::Infof("OruxDBTileDataSource::loadTile: Tile data doesn't exist in the database");
                    return std::shared_ptr<TileData>();
                }
                return tileData;
            }
            ...
            Log::Errorf("OruxDBTileDataSource::loadTile: Failed to query tile data from the database: %s", ex.what());
            return std::shared_ptr<TileData>();
        }
    }

    std::unique_ptr<sqlite3pp::database> OruxDBTileDataSource::OpenDatabase(const std::string& path) {
    ...
    bool OruxDBTileDataSource::loadZoomLevels(int& minZoom, int& maxZoom) const {
        ...
        try {
            ...
            Log::Errorf("OruxDBTileDataSource: Exception while reading zoom level metadata: %s", ex.what());
        }

        // If either was not found, do table scan
        if (!foundMinZoom || !foundMaxZoom) {
            try {
                Log::Warnf("OruxDBTileDataSource: minzoom and maxzoom not found int metadata, we need to compute it and it might be long");
                sqlite3pp::query query(*_database, "SELECT MIN(z), MAX(z) FROM tiles");
                for (auto it = query.begin(); it != query.end(); it++) {
                    if (!foundMinZoom) {
                ...
                Log::Errorf("OruxDBTileDataSource: Failed to read min/max zoom info: %s", ex.what());
                return false;
            }
        }
        return true;
    ...
    bool OruxDBTileDataSource::loadDataExtent(MapBounds& mapBounds) const {
        ...
        try {
            ...
            Log::Errorf("OruxDBTileDataSource::getDataExtent: Exception while reading bounds metadata: %s", ex.what());
        }
        ...
        if (!foundMapBounds) {
            try {
                ...
                Log::Errorf("OruxDBTileDataSource::getDataExtent: Failed to query tile data from the database: %s", ex.what());
                return false;
            }
        }
        return true;
